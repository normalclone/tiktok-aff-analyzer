...
function initFilters(){
  ...
  mk('fShop',render).setOptions(uniq('shop'),'Tất cả kênh');
...
function renderSummary(D,A){
  ...
  if(ineRate>=0.1)
    F(ineRate>=0.2?'bad':'warn',
      `<b>Nhiều đơn bị loại — đây là chỗ mất tiền nhiều nhất.</b> Có tới <b>${p1(ineRate)}</b> đơn không được tính hoa hồng, tương đương khoảng <b>${money(ineLost)}</b> bị mất${ineTopShop?`, nhiều nhất ở kênh <b>${esc(ineTopShop.k)}</b>`:''}. Thường là do khách huỷ/trả hàng, hoặc cách gắn link và nội dung chưa đúng quy định của TikTok.`);

  if(refundRate>=0.05)
    F(refundRate>=0.1?'bad':'warn',
      `<b>Tỷ lệ trả hàng hơi cao.</b> Khoảng <b>${(refundRate*100).toFixed(1)}%</b> số món bị trả lại (${fmt(A.refund)} món)${refundShop&&refundShop.rate>=0.1?`, nhiều nhất ở kênh <b>${esc(refundShop.k)}</b> (${(refundShop.rate*100).toFixed(1)}%)`:''}. Mỗi món bị trả là một lần mất khoản hoa hồng đã tính.`);
  else if(A.sold>0)
    F('good',`<b>Trả hàng ít</b> (${(refundRate*100).toFixed(1)}%) — rất tốt.`);

  if(nShop>=2 && top4Share>0.6)
    F(top4Share>0.8?'bad':'warn',
      `<b>Doanh số phụ thuộc vào ít kênh.</b> ${Math.min(4,nShop)} kênh lớn nhất chiếm tới <b>${p1(top4Share)}</b> doanh số, riêng kênh <b>${esc(topShop.k)}</b> đã <b>${p1(top1Share)}</b>. Nếu một trong số này bán chậm lại, thu nhập sẽ giảm theo ngay.`);
  ...
  if(ineRate>=0.1 && ineLost>0)
    R(1,`<b>Xử lý nhóm đơn bị loại trước tiên.</b> Xem lại vì sao đơn bị loại (khách huỷ/trả hàng, hay gắn link sai cách)${ineTopShop?`, bắt đầu từ kênh <b>${esc(ineTopShop.k)}</b>`:''}. Giảm được phần này là có thể lấy lại tới <b>${money(ineLost)}</b> hoa hồng.`);
  if(refundShop && refundShop.rate>=0.1)
    R(1,`<b>Trao đổi với kênh ${esc(refundShop.k)} để giảm trả hàng.</b> Kiểm tra lại size, hình ảnh và mô tả sản phẩm cho đúng — đây là cách dễ nhất để bớt mất hoa hồng.`);
  if(top4Share>0.7 && nShop>4)
    R(2,`<b>Tìm thêm kênh mới để hợp tác.</b> Đừng chỉ dựa vào vài kênh lớn — trải rộng ra thì thu nhập sẽ ổn định hơn.`);
  if(liveShare>0.9)
  ...
  if(topShop.gmv>0)
    R(3,`<b>Học theo cách làm của kênh ${esc(topShop.k)}</b> (đang hiệu quả nhất) — cùng khung giờ live, cách dẫn dắt, nhóm sản phẩm — rồi áp dụng cho các kênh khác.`);
...
function renderShops(D){
  ...
  document.getElementById('tblShop').innerHTML=
    `<thead><tr><th>Kênh</th><th>Đơn</th><th>GMV</th><th>HH thực nhận</th><th>Hoàn</th><th>% hoàn</th></tr></thead><tbody>`+
...
function renderIneligible(D){
  ...
  document.getElementById('tblIne').innerHTML=
    `<thead><tr><th>Kênh</th><th>Số đơn</th><th>GMV</th><th>HH ước tính mất</th></tr></thead><tbody>`+
...
function renderGap(D){
  ...
  document.getElementById('tblGap').innerHTML=
    `<thead><tr><th>Kênh</th><th>HH ước tính</th><th>HH thực nhận</th><th>Hao hụt</th><th>Hiệu suất</th></tr></thead><tbody>`+
...
function exportCSV(){
  ...
  push('');
  push('THEO KÊNH','Đơn','GMV','HH thực nhận','Món hoàn');
