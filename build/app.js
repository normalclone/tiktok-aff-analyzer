...
function handleFile(file){
  ...
  reader.onerror=()=>fail('Không đọc được file.');
  reader.onload=e=>{
    setLoadingSub('Đang phân tích '+(file.name||'')+'…');
    ...
    setTimeout(()=>{
      try{
        ...
        META.fileName=file.name; META.totalRows=RAW.length;
        initFilters(); render();
...
function renderSummary(D,A){
  ...
  // ===== Đoạn mở: bức tranh chung + dòng hoa hồng ước tính → thực nhận =====
  let intro=`Trong phạm vi đang xem, cửa hàng/nhà sáng tạo đẩy được <b>${money(A.gmv)}</b> GMV qua <b>${fmt(A.orders)}</b> đơn — `+
    `quy mô không nhỏ. Nhưng phần thưởng thực tế lại là câu chuyện khác: hoa hồng kỳ vọng vào khoảng <b>${money(A.est)}</b>, `;
  if(gap>0)
    intro+=`trong khi mới cầm chắc trong tay <b>${money(A.act)}</b>. Khoảng <b>${money(gap)}</b> còn lại (~${p1(1-eff)} kỳ vọng) vẫn đang treo lơ lửng hoặc đã bị gạch bỏ. `;
  else
    intro+=`và gần như đã về đủ — dòng tiền rất “sạch”. `;
  let verdict;
  if(ineRate>=0.2 || eff<0.6) verdict=`Nói thẳng: bán được hàng không phải vấn đề, <b>giữ được hoa hồng</b> mới là chỗ đang chảy máu.`;
  else if(top4Share>0.8 || liveShare>0.95) verdict=`Bức tranh nhìn chung tốt, chỉ có điều nguồn thu đang hơi “đặt nhiều trứng vào một giỏ”.`;
  else verdict=`Nhìn tổng thể, guồng máy đang chạy ổn — còn vài chỗ để siết cho chặt hơn.`;
  intro+=verdict;
  ...
  if(settleRate<0.7)
    F(settleRate<0.5?'bad':'warn',
      `<b>Dòng tiền về chậm.</b> Mới <b>${p1(settleRate)}</b> số dòng đơn được quyết toán, còn <b>${fmt(A.rows-A.settled)}</b> dòng đang treo hoặc bị loại — đây là lý do chính khiến thực nhận thấp hơn hẳn kỳ vọng.`);
  else
    F('good',`<b>Dòng tiền lành mạnh.</b> Phần lớn đơn đã quyết toán (${p1(settleRate)}), hoa hồng về đều đặn.`);

  if(ineRate>=0.1)
    F(ineRate>=0.2?'bad':'warn',
      `<b>Đơn “không đủ điều kiện” là điểm rò rỉ lớn nhất.</b> Nhóm này chiếm tới <b>${p1(ineRate)}</b> và kéo theo khoảng <b>${money(ineLost)}</b> hoa hồng${ineTopShop?`, dồn nhiều ở <b>${esc(ineTopShop.k)}</b>`:''}. Thủ phạm quen thuộc thường là huỷ/hoàn hàng hoặc cách gắn link, nội dung chưa khớp chính sách.`);

  if(refundRate>=0.05)
    F(refundRate>=0.1?'bad':'warn',
      `<b>Tỷ lệ hoàn còn cấn.</b> Ở mức <b>${(refundRate*100).toFixed(1)}%</b> (${fmt(A.refund)} món)${refundShop&&refundShop.rate>=0.1?`, gắt nhất ở <b>${esc(refundShop.k)}</b> (${(refundShop.rate*100).toFixed(1)}%)`:''} — mỗi đơn hoàn là một lần hoa hồng đã chốt rồi lại bốc hơi.`);
  else if(A.sold>0)
    F('good',`<b>Hoàn hàng trong tầm kiểm soát</b> (${(refundRate*100).toFixed(1)}%).`);

  if(nShop>=2 && top4Share>0.6)
    F(top4Share>0.8?'bad':'warn',
      `<b>Doanh số dồn vào số ít cửa hàng.</b> Top ${Math.min(4,nShop)} đã gánh <b>${p1(top4Share)}</b> GMV, riêng <b>${esc(topShop.k)}</b> chiếm <b>${p1(top1Share)}</b> — chỉ cần một nguồn này hụt hơi là tổng thu rung lắc theo.`);

  if(liveShare>0.9)
    F('warn',`<b>Sống nhờ Live.</b> Gần như toàn bộ đơn (${p1(liveShare)}) đến từ livestream; video ngắn và trang trưng bày gần như chưa được tận dụng để bán ngoài giờ phát.`);

  F(topProdShare>0.4?'warn':'good',
    topProdShare>0.4
      ? `<b>Phụ thuộc một mã chủ lực.</b> Riêng “${esc(clip(topProd.name,40))}” đã chiếm ${p1(topProdShare)} GMV trên tổng ${fmt(nProd)} sản phẩm.`
      : `<b>Danh mục đủ rộng.</b> ${fmt(nProd)} sản phẩm và không quá lệ thuộc một mã (mã mạnh nhất chỉ ${p1(topProdShare)} GMV) — nền tảng tốt để mở rộng.`);

  // ===== Nên làm gì tiếp theo =====
  const recos=[];
  const R=(p,html)=>recos.push({p,html});
  if(ineRate>=0.1 && ineLost>0)
    R(1,`<b>Cứu nhóm đơn không đủ điều kiện trước tiên.</b> Soát lại quy trình huỷ/hoàn và cách gắn link${ineTopShop?`, bắt đầu từ <b>${esc(ineTopShop.k)}</b>`:''}; kéo giảm được một phần thôi cũng có thể thu lại tới <b>${money(ineLost)}</b>.`);
  if(refundShop && refundShop.rate>=0.1)
    R(1,`<b>Ngồi lại với ${esc(refundShop.k)}</b> về size, ảnh và cam kết giao hàng để hạ tỷ lệ hoàn — đây là khoản rò rỉ hoa hồng dễ bịt nhất.`);
  if(top4Share>0.7 && nShop>4)
    R(2,`<b>Mở rộng tệp cửa hàng</b> ngoài nhóm dẫn đầu, để nồi cơm bớt phụ thuộc một vài nguồn.`);
  if(liveShare>0.9)
    R(2,`<b>Tận dụng video ngắn và trang trưng bày</b> để có đơn đều đặn ngoài khung giờ live, kéo dài tuổi thọ mỗi nội dung.`);
  if(pending.length>0 && pendEst>0)
    R(2,`<b>Bám sát ${fmt(pending.length)} đơn đang treo</b> (~${money(pendEst)} hoa hồng) cho tới hết thời gian đối soát để chốt về thực nhận.`);
  if(topProdShare>0.4 && topProd)
    R(2,`<b>Bán kèm/upsell quanh mã chủ lực</b> “${esc(clip(topProd.name,38))}” để không bỏ trứng vào một giỏ.`);
  if(topShop.gmv>0)
    R(3,`<b>Nhân bản công thức của ${esc(topShop.k)}</b> — khung giờ, kịch bản live, nhóm hàng đang ăn — sang các cửa hàng còn lại.`);
  R(3,`<b>Dồn lực cho hàng “vừa lãi vừa ít hoàn”</b>: ưu tiên ngân sách cho SKU hoa hồng tốt, hoàn thấp; mạnh tay cắt mã hoàn nhiều.`);

  // ===== Render 1 card prose =====
  let html=`<p>${intro}</p>`;
  html+=`<div class="sub-h"><span class="ico">${icon('activity')}</span>Bức tranh hiện tại</div>`+
